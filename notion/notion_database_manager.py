...
def _calculate_realized_pnl(market: str, snapshot_date: str, platform: str | None = None) -> dict:
    """
    Calculate both daily and cumulative realized P&L in a single pass.

    The lot engine uses the conservative model: sells consume the lowest-cost
    lots first, and buy fees are included in unit cost.

    Returns dict with keys:
        realized_pnl            — P&L from trades on snapshot_date only
        realized_trade_count    — number of sell/fee trades on snapshot_date
        cumulative_realized_pnl — P&L from all trades up to snapshot_date
        cumulative_cost_basis   — total cost basis of all buys up to snapshot_date
        errors                  — list of error messages per stock group
    """
    ...
    for key, trades in grouped_trades.items():
        ...
        try:
            for trade in trades:
                ...
                if trade["Action"] == "Buy":
                    if trade["Count"] == 0:
                        # Fee-only trade (e.g. dividend tax, platform fee)
                ...
                trade_pnl = (trade["Count"] * trade["Price"]) - trade["Trade Fee"] - cost_basis
    ...
    try:
        ...
        for current_market in markets:
            ...
            for current_platform in _daily_pnl_platforms(current_market, platform):
                ...
                realized = _calculate_realized_pnl(
                    current_market,
                    current_date,
                    current_platform,
                )
                ...
                cumulative_realized_pnl = realized["cumulative_realized_pnl"]
                cumulative_cost_basis = realized["cumulative_cost_basis"]
                realized_errors = realized["errors"]
